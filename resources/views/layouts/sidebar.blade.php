...
<aside id="sidebar"
            ...
            @foreach ($menuGroup['items'] as $itemIndex => $item)
                @if (isset($item['subItems']))
                    @foreach ($item['subItems'] as $subItem)
                        if (currentRoute === '{{ $subItem['route_name'] }}') {
 ...
 @mouseleave="$store.sidebar.setHovered(false)">
    ...
    <div class="pt-8 pb-7 flex"
        ...
        <a href="{{ Route::has('admin.dashboard') && auth()->user()?->role === 'admin' ? route('admin.dashboard') : route('dashboard') }}" class="flex items-center">
                            ...
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                <li>
                                    @if (isset($item['subItems']))
                                        ...
                                        <div x-show="isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) && ($store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen)" x-cloak>
                                            <ul class="mt-2 space-y-1 ml-9">
                                                @foreach ($item['subItems'] as $subItem)
                                                    <li>
                                                        <a href="{{ route($subItem['route_name']) }}" class="menu-dropdown-item"
                                                            :class="isActive('{{ $subItem['route_name'] }}') ? 'menu-dropdown-item-active' : 'menu-dropdown-item-inactive'">
                                                            {{ $subItem['name'] }}
                                                        </a>
                                                    </li>
                                                @endforeach
                                    ...
                                    @else
                                        <a href="{{ route($item['route_name']) }}" class="menu-item group"
                                            :class="[
                                                isActive('{{ $item['route_name'] }}') ? 'menu-item-active' : 'menu-item-inactive',
                                                (!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ? 'xl:justify-center' : 'justify-start'
                                            ]">
                                            <span :class="isActive('{{ $item['route_name'] }}') ? 'menu-item-icon-active' : 'menu-item-icon-inactive'">
                                                {!! MenuHelper::getIconSvg($item['icon']) !!}
                                            </span>
                                            <span x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen" class="menu-item-text flex items-center gap-2">
                                                {{ $item['name'] }}
