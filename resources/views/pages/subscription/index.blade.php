    ...
    <div class="bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 min-h-screen py-10 transition-colors duration-200">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
            ...
            <header class="border-b border-gray-200 dark:border-gray-800 pb-6">
                <h1 class="text-3xl font-extrabold tracking-tight sm:text-4xl">Mon abonnement</h1>
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Gérez votre abonnement, vos paiements et vos factures.</p>
            </header>
            ...
            @if($subscription && $subscription->valid())
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <section class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xs p-6 relative overflow-hidden" aria-labelledby="billing-summary-title">
                            <div class="absolute top-0 right-0 p-4">
                                @if($subscription->onTrial())
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800">Essai</span>
                                @elseif($subscription->canceled())
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-800">Résilié</span>
                                @elseif($user->subscription('default')->pastDue())
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800">Paiement en retard</span>
                                @else
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-800">Actif</span>
                                @endif
                            </div>

                            <h2 id="billing-summary-title" class="text-lg font-bold tracking-tight mb-4">Détails de l'offre</h2>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 text-sm">
                                ...
                                <div>
                                    <p class="text-gray-500 dark:text-gray-400">Identifiant Client</p>
                        ...
                        <section class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xs p-6" aria-labelledby="billing-cycle-title">
                            <div class="flex justify-between items-baseline mb-2">
                                <h3 id="billing-cycle-title" class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Cycle de facturation actuel</h3>
                                <span class="text-xs text-gray-500 dark:text-gray-400 font-medium">{{ $daysUsed }} / {{ $totalDays }} jours consommés</span>
                            </div>
                            ...
                            <div class="mt-4 flex flex-wrap justify-between items-center text-sm gap-2">
                                <p class="text-gray-600 dark:text-gray-400">Il reste <span class="font-semibold text-gray-900 dark:text-white">{{ $daysRemaining }} jours</span> avant la prochaine échéance.</p>
                                <span class="text-xs px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded font-medium">Prochain prélèvement le {{ $nextPaymentDate }}</span>
                            </div>
                        </section>
                    ...
                    <div>
                        <section class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xs p-6 h-full flex flex-col justify-between" aria-labelledby="account-state-title">
                            <div>
                                <h2 id="account-state-title" class="text-lg font-bold tracking-tight mb-4">Statut de l'espace Ilands</h2>
                                ...
                                <div class="space-y-4">
                                    <div class="flex items-center gap-2.5 text-sm text-green-600 dark:text-green-400 font-medium">
                                        ...
                                        Abonnement actif : Fonctionnalités premium débloquées
                                    </div>
                                    ...
                                    <div class="pt-4 border-t border-gray-100 dark:border-gray-800">
                                        ...
                                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Quota restant : {{ $usageMetrics['total'] - $usageMetrics['used'] }} ressources</p>
                                    </div>
                                </div>
                            </div>
                            
                            <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500 dark:text-gray-400">
                                Des questions ? Contactez le support support@ilands.io
                            </div>
                        </section>
                    </div>
                </div>
                ...
                <section aria-labelledby="quick-actions-title">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <a href="{{ route('subscription.billing') }}" class="group bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-5 rounded-xl shadow-xs transition-all duration-200 hover:scale-[1.02] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 flex items-center gap-4" aria-label="Gérer les paramètres globaux de l'abonnement sur Stripe">
                            ...
                            <div>
                                <h3 class="font-semibold text-sm">Gérer l'abonnement</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Modifier ou faire évoluer l'offre</p>
                            </div>
                        </a>
                        ...
                        <a href="{{ route('subscription.billing') }}" class="group bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-5 rounded-xl shadow-xs transition-all duration-200 hover:scale-[1.02] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 flex items-center gap-4" aria-label="Mettre à jour la carte bancaire sur Stripe">
                            ...
                            <div>
                                <h3 class="font-semibold text-sm">Mettre à jour la carte</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Changer de moyen de paiement</p>
                            </div>
                        </a>
                        ...
                        <a href="#invoices-section" class="group bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-5 rounded-xl shadow-xs transition-all duration-200 hover:scale-[1.02] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 flex items-center gap-4" aria-label="Faire défiler jusqu'à l'historique des factures">
                            ...
                            <div>
                                <h3 class="font-semibold text-sm">Voir les factures</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Historique des transactions</p>
                            </div>
                        </a>
                        ...
                        <a href="{{ route('subscription.billing') }}" class="group bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-5 rounded-xl shadow-xs transition-all duration-200 hover:scale-[1.02] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-500 flex items-center gap-4" aria-label="Résilier votre forfait actuel sur Stripe">
                            ...
                            <div>
                                <h3 class="font-semibold text-sm">Annuler l'abonnement</h3>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Résilier le forfait</p>
                            </div>
                        </a>
                    </div>
                </section>
                ...
                <section id="invoices-section" class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xs overflow-hidden" aria-labelledby="invoices-title">
                    <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-800">
                        <h2 id="invoices-title" class="text-lg font-bold tracking-tight">Historique des factures</h2>
                    </div>
                            ...
                            <table class="w-full text-left border-collapse text-sm">
                                <thead>
                                    <tr class="bg-gray-50 dark:bg-gray-950/50 text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800">
                                        <th scope="col" class="px-6 py-3.5 font-semibold">Date de facturation</th>
                                        <th scope="col" class="px-6 py-3.5 font-semibold">Montant</th>
                                        <th scope="col" class="px-6 py-3.5 font-semibold">Statut</th>
                                        <th scope="col" class="px-6 py-3.5 font-semibold text-right">Facture PDF</th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach($invoices as $invoice)
                                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <a href="{{ route('cashier.invoice', $invoice->id) }}" target="_blank" class="inline-flex items-center gap-1.5 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 focus-visible:underline" aria-label="Télécharger la facture du {{ $invoice->date()->format('d M Y') }} au format PDF">
                                                    Télécharger
